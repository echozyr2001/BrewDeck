        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{stats.pending + stats.running}</p>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          {filteredOperations.length === 0 ? (
            ...
            filteredOperations.map((operation, index) => (
              <div key={operation.id}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium">
                        {operation.type.charAt(0).toUpperCase() + operation.type.slice(1)} {operation.packageName}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {new Date(operation.startTime).toLocaleString()} • {formatDuration(operation)}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                {index < filteredOperations.length - 1 && <Separator className="my-2" />}
              </div>
            ))
          )}
        </div>
...
export const OperationStatusManager: React.FC = () => {
  ...
  const handleRetryOperation = async (operation: PackageOperation) => {
    switch (operation.type) {
      case 'install':
        await installPackage(operation.packageName, operation.packageType);
        break;
      case 'uninstall':
        await uninstallPackage(operation.packageName, operation.packageType);
        break;
      case 'update':
        await updatePackage(operation.packageName, operation.packageType);
        break;
    }
  };
    ...
    <div className="space-y-6">
      ...
      {hasActiveOperations && (
        <Card>
          ...
          <CardContent>
            <OperationQueue maxVisible={10} showCompleted={false} compact={false} />
          </CardContent>
        </Card>
      )}
