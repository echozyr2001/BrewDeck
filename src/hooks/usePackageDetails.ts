...
import type { EnhancedBrewPackage, PackageWarning } from '../stores/brewStore';

/**
 * Custom hook for detailed package information
 * Provides comprehensive package data and analysis
 */
export const usePackageDetails = (packageName: string, packageType: 'formula' | 'cask') => {
  const {
    formulae,
    casks,
    getPackagesByType,
  ...
  const operationStatus = useMemo(() => {
    ...
    return activeOps.find(op => 
      op.packageName === packageName && op.packageType === packageType
    );
  }, [getActiveOperations, packageName, packageType]);

  // Check if package has active operation
  const hasActiveOperation = useMemo(() => !!operationStatus, [operationStatus]);
  ...
  const getConflictDetails = useCallback(() => {
    ...
    return packageData.conflicts.map(conflictName => {
      ...
      return {
        ...
        severity: conflictPackage?.installed ? 'high' : 'low' as 'high' | 'low',
      };
    });
  }, [packageData, packageType, getPackagesByType]);
  ...
  const getWarningAnalysis = useCallback(() => {
    if (!packageData?.warnings) return { total: 0, bySeverity: {}, byType: {} };
    ...
    packageData.warnings.forEach(warning => {
      analysis.bySeverity[warning.severity] = (analysis.bySeverity[warning.severity] || 0) + 1;
  ...
  const getSafetyAssessment = useCallback(() => {
    ...
    if (warnings.critical.length > 0) {
      safetyLevel = 'danger';
      issues.push(`${warnings.critical.length} critical security/compatibility issues`);
    } else if (installedConflicts.length > 0) {
      safetyLevel = 'warning';
      issues.push(`Conflicts with ${installedConflicts.length} installed packages`);
  ...
  const getInstallationRecommendations = useCallback(() => {
    ...
    if (missingDeps.length > 0) {
      recommendations.push(`Install ${missingDeps.length} missing dependencies first`);
    }
