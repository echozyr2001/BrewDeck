...
export const useOperationQueue = () => {
  ...
  const pauseQueue = useCallback(() => {
    const pendingOps = Object.values(operations).filter(
  ...
  const retryFailedOperations = useCallback(async () => {
    const failedOps = Object.values(operations).filter(
      (op) => op.status === "failed"
    );
  ...
  const getQueueHealth = useCallback(() => {
    ...
    const successRate =
      totalFinished > 0 ? (stats.completed / totalFinished) * 100 : 100;
  ...
  const operationsByPriority = useMemo(
